...
RSpec.feature "課金 プランアップグレード", type: :feature do

  let(:mail_address)      { 'test@request.com' }
  let(:company_name)      { '太陽株式会社' }
  ...
  xscenario '課金 -> アップグレード -> 次に定期更新', js: true do
    ...
    expect(ActionMailer::Base.deliveries.first.body.raw_source).to match(/変更前プラン名: Rspecテスト ライトプラン/)
    ...
    expect(his.memo).to match /#{EasySettings.plan[:test_light]}\":\"/
    expect(his.start_at).to eq user.billing.reload.first_paid_at.beginning_of_day.iso8601
    expect(his.end_at).to eq user.billing.expiration_date.iso8601
