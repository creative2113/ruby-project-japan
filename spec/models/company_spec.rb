...
RSpec.describe Company, type: :model do
  ...
  describe '#select_by_connectors & #select_domain_by_connectors' do
    ...
    let_it_be(:company12) { create(:company, domain: 'urayasu_2, tsushin_2') }
    let_it_be(:company13) { create(:company, domain: 'sinjuku_3, chigin_2') }
    let_it_be(:company14) { create(:company, domain: 'aichi, seizou') }
    let_it_be(:company15) { create(:company, domain: 'kobe_1,') }
    let_it_be(:company16) { create(:company, domain: 'sakana_2,') }
    ...
    let_it_be(:company_area12) { create(:company_area_connector, company: company12, area_connector: area_connector10) }
    let_it_be(:company_area13) { create(:company_area_connector, company: company13, area_connector: area_connector3) }
    let_it_be(:company_area14) { create(:company_area_connector, company: company14, area_connector: area_connector15) }
    let_it_be(:company_area15) { create(:company_area_connector, company: company15, area_connector: area_connector32) }
    ...
    let_it_be(:company_category12) { create(:company_category_connector, company: company13, category_connector: category_connector17) }
    let_it_be(:company_category13) { create(:company_category_connector, company: company14, category_connector: category_connector1) }
    let_it_be(:company_category14) { create(:company_category_connector, company: company16, category_connector: category_connector13) }
    ...
    let(:area_connector_ids) { [] }
    let(:category_connector_ids) { [] }
    let(:capital_ids) { [] }
    let(:employee_ids) { [] }
    let(:sales_ids) { [] }

    let(:not_own_capitals) { false }

    let(:select_by_connectors_response) { described_class.select_by_connectors(area_connector_ids, category_connector_ids, capital_ids, employee_ids, sales_ids, not_own_capitals) }
    let(:select_domain_by_connectors_response) { described_class.select_domain_by_connectors(area_connector_ids, category_connector_ids, capital_ids, employee_ids, sales_ids, not_own_capitals) }

    let(:expect_companies) { [] }
    let(:expect_company_domains) { expect_companies.map { |comp| comp.domain } }

    context 'エリアに関して' do
      context 'その1' do
        let(:area_connector_ids) { [area_connector3.id] }
        let(:expect_companies) { [company1, company2, company13] }
        it { expect(select_by_connectors_response).to eq expect_companies }
        it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
      end

      context 'その2' do
        let(:area_connector_ids) { [area_connector28.id, area_connector4.id] }
        let(:expect_companies) { [company3, company4, company5, company9, company10] }
        it { expect(select_by_connectors_response).to eq expect_companies }
        it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
      end

      context 'その3' do
        let(:area_connector_ids) { [area_connector8.id, area_connector24.id] }
        let(:expect_companies) { [company6, company7, company8, company9, company10, company11, company12, company15] }
        it { expect(select_by_connectors_response).to eq expect_companies }
        it { expect(select_domain_by_connectors_response).to eq expect_company_domains }

        context '業種も条件にある時1' do
          let(:category_connector_ids) { [category_connector18.id, category_connector13.id] }
          let(:expect_companies) { [company6, company9] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end

        context '業種も条件にある時2' do
          let(:category_connector_ids) { [category_connector2.id] }
          let(:expect_companies) { [] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end
    ...
    context '業種に関して' do
      ...
      context 'その1' do
        let(:category_connector_ids) { [category_connector2.id] }
        let(:expect_companies) { [company1, company2] }
        it { expect(select_by_connectors_response).to eq expect_companies }
        it { expect(select_domain_by_connectors_response).to eq expect_company_domains }

        context 'エリアも条件にある時1' do
          let(:area_connector_ids) { [area_connector8.id] }
          let(:expect_companies) { [] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end

        context 'エリアも条件にある時2' do
          let(:area_connector_ids) { [area_connector2.id] }
          let(:expect_companies) { [company1, company2] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end
      ...
      context 'その2' do
        let(:category_connector_ids) { [category_connector16.id, category_connector10.id] }
        let(:expect_companies) { [company6, company8, company13, company16] }
        it { expect(select_by_connectors_response).to eq expect_companies }
        it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
      end

      context 'その3' do
        let(:category_connector_ids) { [category_connector1.id, category_connector19.id] }
        let(:expect_companies) { [company1, company2, company3, company4, company5, company11, company12, company14] }
        it { expect(select_by_connectors_response).to eq expect_companies }
        it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
      
        context 'エリアも条件にある時1' do
          let(:area_connector_ids) { [area_connector4.id, area_connector31.id, area_connector25.id] }
          let(:expect_companies) { [company3, company4, company5, company11] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end
      end
    ...
    context '資本金に関して' do
      context 'ケース1' do
        let(:capital_ids) { [capital_group1.id] }
        let(:expect_companies) { [company1, company3, company5, company9] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
      end

      context 'ケース2' do
        let(:capital_ids) { [capital_group3.id, capital_group2.id] }
        let(:expect_companies) { [company2, company6, company7, company11] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
      end

      context 'エリアも条件にある時' do
        let(:area_connector_ids) { [area_connector8.id] }
        let(:capital_ids) { [capital_group3.id, capital_group2.id] }
        let(:expect_companies) { [company6, company7] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
      end
    end
    ...
    context '従業員数に関して' do
      context 'ケース1' do
        let(:employee_ids) { [employee_group4.id] }
        let(:expect_companies) { [company3, company6, company11] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
      end

      context 'ケース2' do
        let(:employee_ids) { [employee_group4.id, employee_group2.id] }
        let(:expect_companies) { [company3, company6, company11, company1, company5, company7, company8] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }

        context 'カテゴリも条件にある時' do
          let(:category_connector_ids) { [category_connector1.id] }
          let(:expect_companies) { [company3, company1, company5] }
          it { expect(select_by_connectors_response).to match_array expect_companies }
          it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
        end

        context '売上も条件にある時' do
          let(:sales_ids) { [sales_group1.id, sales_group4.id] }
          let(:expect_companies) { [company1, company5, company6, company11] }
          it { expect(select_by_connectors_response).to match_array expect_companies }
          it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
        end
      end
    ...
    context '売上に関して' do
      context 'ケース1' do
        let(:sales_ids) { [sales_group4.id] }
        let(:expect_companies) { [company1, company2, company6, company11] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
      end

      context 'ケース2' do
        let(:sales_ids) { [sales_group4.id, sales_group1.id] }
        let(:expect_companies) { [company1, company2, company4, company5, company6, company11] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }

        context '資本金も条件にある時' do
          let(:capital_ids) { [capital_group3.id, capital_group1.id] }
          let(:expect_companies) { [company1, company5, company11] }
          it { expect(select_by_connectors_response).to match_array expect_companies }
          it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
        end
      end

      context '資本金も従業員数も条件にある時' do
        let(:capital_ids) { [capital_group3.id, capital_group1.id] }
        let(:employee_ids) { [employee_group3.id, employee_group4.id] }
        let(:sales_ids) { [sales_group1.id, sales_group3.id] }
        let(:expect_companies) { [company3, company9] }
        it { expect(select_by_connectors_response).to match_array expect_companies }
        it { expect(select_domain_by_connectors_response).to match_array expect_company_domains }
      end
    end

    context 'not_own_capitalsの場合' do
      let(:not_own_capitals) { true }

      context 'エリアに関して' do
        context 'その1' do
          let(:area_connector_ids) { [area_connector3.id] }
          let(:expect_companies) { [company13] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end
        ...
        context 'その3' do
          let(:area_connector_ids) { [area_connector8.id, area_connector24.id] }
          let(:expect_companies) { [company12, company15] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
          ...
          context '業種も条件にある時2' do
            let(:category_connector_ids) { [category_connector19.id, category_connector6.id] }
            let(:expect_companies) { [company12] }
            it { expect(select_by_connectors_response).to eq expect_companies }
            it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
          end
        end
      ...
      context '業種に関して' do
        context 'その1' do
          let(:category_connector_ids) { [category_connector16.id, category_connector10.id] }
          let(:expect_companies) { [company13, company16] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
        end
        ...
        context 'その2' do
          let(:category_connector_ids) { [category_connector1.id, category_connector19.id] }
          let(:expect_companies) { [company12, company14] }
          it { expect(select_by_connectors_response).to eq expect_companies }
          it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
          ...
          context 'エリアも条件にある時1' do
            let(:area_connector_ids) { [area_connector1.id] }
            let(:expect_companies) { [company12] }
            it { expect(select_by_connectors_response).to eq expect_companies }
            it { expect(select_domain_by_connectors_response).to eq expect_company_domains }
          end
